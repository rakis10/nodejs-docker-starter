...
const db = new sqlite3.Database('./db.sqlite', err => {
  ...
  //   }
  // });
  // create init check for category table 
  db.get('SELECT name FROM sqlite_master WHERE type="table" AND name="category"', (err, row) => {
    if (err) {
      console.error('Error checking table existence:', err.message);
    } else if (!row) {
      // Table doesn't exist, execute init.sql statements
      ...
      console.log('Table "category" already exists, skipping initialization.');
    }
  });
